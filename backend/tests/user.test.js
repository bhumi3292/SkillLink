...
describe("Auth, Category, and Basic Property API Tests", () => {
    ...
    beforeAll(async () => {
        ...
        expect(workerRegisterRes.statusCode).toBe(201);
        ...
        expect(workerLoginRes.statusCode).toBe(200);
...
// --- Cart API Tests (using SkillLink_test_cart) ---
describe("Cart API Tests", () => {
    // beforeAll for Cart API tests
    beforeAll(async () => {
        process.env.MONGO_URI = "mongodb://localhost:27017/SkillLink_test_cart";

        // Ensure disconnection from any previous connection if it's not the correct DB
        if (mongoose.connection.readyState === 1 && mongoose.connection.name !== 'SkillLink_test_cart') {
            await mongoose.disconnect();
            console.log("Disconnected from previous non-cart DB connection.");
        }
        // Connect only if not already connected to the correct DB
        if (mongoose.connection.readyState === 0 || mongoose.connection.name !== 'SkillLink_test_cart') {
            await mongoose.connect(process.env.MONGO_URI);
        ...
        // Register and login a Hirer user for cart operations
        const HirerRegisterRes = await request(app).post("/api/auth/register").send({
            fullName: "Cart Test Hirer",
            email: "Hirer@cart.com",
            phoneNumber: "9811111111",
            stakeholder: "Hirer",
            password: "password123",
            confirmPassword: "password123",
        });
        expect(HirerRegisterRes.statusCode).toBe(201);

        const HirerLoginRes = await request(app).post("/api/auth/login").send({
            email: "Hirer@cart.com",
            password: "password123",
        });
        expect(HirerLoginRes.statusCode).toBe(200);
        HirerTokenCartDB = HirerLoginRes.body.token;

        const HirerUser = await User.findOne({ email: "Hirer@cart.com" });
        testHirerIdCartDB = HirerUser._id;

        // Register and login a worker user specific to this cart test DB
        const workerRegisterResForCart = await request(app).post("/api/auth/register").send({
            fullName: "worker For Cart Tests",
            email: "worker@cartdb.com",
            phoneNumber: "9833333333",
            stakeholder: "worker",
            password: "password123",
            confirmPassword: "password123",
        });
        expect(workerRegisterResForCart.statusCode).toBe(201);

        const workerLoginResForCart = await request(app).post("/api/auth/login").send({
            email: "worker@cartdb.com",
            password: "password123",
        });
    ...
    test("should return 404 when trying to clear a non-existent cart", async () => {
        ...
        await Cart.deleteOne({ user: testHirerIdCartDB });
