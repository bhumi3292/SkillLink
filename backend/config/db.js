...
const connectDB = async (testDbUri = null) => {
    ...
    if (process.env.NODE_ENV === "test") {
        mongoURI = testDbUri || process.env.MONGO_URI_TEST || "mongodb://localhost:27017/SkillLink_test_db";
        console.log(`Attempting to connect to test MongoDB: ${mongoURI}`);
    } else {
        // For development or production, use the main MONGO_URI
        // Default DB name changed to skillLink_dv per project requirements
        mongoURI = process.env.MONGO_URI || "mongodb://localhost:27017/skillLink_dv";
