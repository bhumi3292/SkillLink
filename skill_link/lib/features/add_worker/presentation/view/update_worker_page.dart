...
class _UpdateWorkerPageState extends State<UpdateWorkerPage> {
  ...
  void initState() {
    ...
    _bedroomsController = TextEditingController(
      text: widget.initialBedrooms.toString(),
    );
    _bathroomsController = TextEditingController(
      text: widget.initialBathrooms.toString(),
    );
  ...
  Future<void> _fetchCategories() async {
    ...
    result.fold((failure) => setState(() => _categories = []), (cats) {
      setState(() {
        _categories = cats;
        _selectedCategoryId ??= widget.initialCategoryId;
      });
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (name.isNotEmpty) {
                    final addCategoryUsecase =
                        serviceLocator<AddCategoryUsecase>();
                    final result = await addCategoryUsecase(
                      CategoryEntity(categoryName: name),
                    );
                    result.fold(
                      (failure) {
                        if (!mounted) return;
  ...
  bool _validate() {
    ...
    } else if (_bedroomsController.text.trim().isEmpty ||
        int.tryParse(_bedroomsController.text) == null ||
        int.parse(_bedroomsController.text) < 0) {
      _errorMessage = 'Valid number of bedrooms is required';
    } else if (_bathroomsController.text.trim().isEmpty ||
        int.tryParse(_bathroomsController.text) == null ||
        int.parse(_bathroomsController.text) < 0) {
  ...
  Future<void> _submitUpdate() async {
    ...
    try {
      ...
      formData.fields.add(
        MapEntry('existingImages', jsonEncode(_existingImages)),
      );
      formData.fields.add(
        MapEntry('existingVideos', jsonEncode(_existingVideos)),
      );
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Workerupdated successfully!'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to update worker: ${response.statusMessage}'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to update worker: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
