...
import 'package:skill_link/features/add_worker/data/data_source/category/category_data_source.dart';
import 'package:skill_link/features/add_worker/domain/entity/category/category_entity.dart'; // Domain entity
...
class CategoryRemoteDatasource implements ICategoryDataSource {
  ...
  Future<List<CategoryEntity>> getCategories() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to get categories: ${response.statusCode} - ${response.data}',
        );
      }
  ...
  Future<void> addCategory(CategoryEntity category) async {
    try {
      ...
      final requestData = {'name': category.categoryName};
      ...
      if (response.statusCode != 201 && response.statusCode != 200) {
        throw Exception(
          'Failed to add category: ${response.statusCode} - ${response.data}',
        );
      }
      ...
      throw Exception(
        'Failed to add category (Dio Error): ${e.response?.data ?? e.message}',
      );
    } catch (e) {
  ...
  Future<void> updateCategory(CategoryEntity category) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Failed to update category: ${response.statusCode} - ${response.data}',
        );
      }
      print('Category updated successfully: ${response.data}');
    } on DioException catch (e) {
      throw Exception(
        'Failed to update category (Dio Error): ${e.response?.data ?? e.message}',
      );
    } catch (e) {
  ...
  Future<void> deleteCategory(String categoryId) async {
    try {
      ...
      ); // API endpoint for deleting by ID

      if (response.statusCode != 200 && response.statusCode != 204) {
        throw Exception(
          'Failed to delete category: ${response.statusCode} - ${response.data}',
        );
      }
      print('Category with ID $categoryId deleted successfully.');
    } on DioException catch (e) {
      throw Exception(
        'Failed to delete category (Dio Error): ${e.response?.data ?? e.message}',
      );
    } catch (e) {
